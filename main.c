...
void show_man_page(const char *path) {
	snprintf(command, sizeof(command), "vim %s", path); system(command);
}
...
pid_t launch_target() {
	...
	if (pid == 0) {
		...
		execl(prog_path_char, prog_path_char, NULL); // child calls to create new process image, execl(path, arg, ..), triggers SIGTRAP for waitpid()
		die("launch_target: exec failed");
	}
...
// popen -> path -> fp -> fgets the output -> put into file by strncpy
void print_source_with_context(unsigned long pc, const char *prog_path) {
	snprintf(command, sizeof(command),"addr2line -e %s 0x%lx", prog_path, pc); // writes formatted output to buffer in memory, addr2line = address to line
	system(command);
	FILE *fp = popen(command, "r"); // launch cmd - pipe in read mode -> program
	if (!fp) return;
	char file[512];
	char output[512];
	int line;
	if (fgets(output, sizeof(output), fp)) {
		char *colon = strrchr(output, ':');  // last occurence of s, as filepath/file.c:line
		if (colon) {
			*colon = '\0'; // reset
			line = atoi(colon + 1); // ascii to int??
			strncpy(file, output, sizeof(file)); // dest, src, size
		}
	}
	pclose(fp);

	FILE *src = fopen(file, "r"); 
	if (!src) { printf("Could not open source file: %s\n", file); return; }
	char buffer[1024];
	int start = line - 2, end = line + 2;
	int current = 1;
	while(fgets(buffer, sizeof(buffer), src)) {
		if (current >= start && current <= end) {
			if (current == line) {
				printf(COLOR_BOLD COLOR_CYAN "=> %d  %s" COLOR_RESET, current, buffer);
			} else { printf("   %d  %s", current, buffer); }
		}
		current++;
	}
	fclose(src);
...
int remove_breakpoint(pid_t pid, unsigned long addr) {
	for (int i = 0; i < bp_count; i++) {
		if (breakpoints[i].active && breakpoints[i].addr == addr) {
			...
			printf(COLOR_RESET COLOR_YELLOW "Breakpoint removed @ 0x%lx\n" COLOR_RESET, addr);
			return 0;
		}
	}
...
void handle_breakpoint(pid_t pid, int quit_proc, char *prog_path) {
	...
	for (int i = 0; i < bp_count; i++) {
		if (breakpoints[i].active && pc/* - 4 */== breakpoints[i].addr) { // instr are 4 bytes. 
			printf(COLOR_YELLOW "Hit breakpoint @ 0x%lx\n" COLOR_RESET, breakpoints[i].addr);
			print_source_with_context(breakpoints[i].addr, prog_path);
...
int main(int argc, char *argv[]) {
	...
	strncpy(prog_path_char, argv[1], sizeof(prog_path_char) - 1);
	
	pid_t pid = launch_target();
	...
	while(!WIFEXITED(status)) { // status macro decoder
		...
		if (strcmp(command, "continue") == 0 || strcmp(command, "c") == 0) {
			...
			if (bp_count) {
				printf(COLOR_YELLOW "Number | Address | Active\n"COLOR_RESET);
			}
			for (int i = 0; i < bp_count; i++) {
			...
			continue;
		} else if (strcmp(command, "restart") == 0 | strcmp(command, "r") == 0) {
			printf("Restarting.\n");
			ptrace(PTRACE_KILL, pid, NULL, NULL);
			waitpid(pid, &status, 0);
			pid = launch_target();
			printf("Reinstall breakpoints? (1/0): "); read_command(command, sizeof(command));
			if (strcmp(command, "1") == 0) {
				printf("Reinstalling all %d total breakpoints.\n", bp_count);
				for (int i = 0; i < bp_count; i++) {
					set_breakpoint(pid, breakpoints[i]. addr);
				}
			}
		} else if (strcmp(command, "man") == 0) {
			show_man_page("./man.txt");
		} else {
